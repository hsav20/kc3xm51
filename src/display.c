...
//MDEBUG(0x99);MDEBUG(change);MDEBUG(gDIP_MenuSelect);MDEBUG(index);
    if (index == gDIP_MenuSelect){                          // 上次已经进入当前菜单
        if (index == cMenu_Surround2Ch){                    // 立体声
    		if (gDIP_Surround[0]){                          // 如果打开超低音
                gDIP_Surround[0] = 0;                       // 关闭超低音
            }else {
                gDIP_Surround[0] = 1;                       // 打开超低音
            }
...
//MDEBUG(0x99);MDEBUG(gDIP_Surround[1]);
            if (gDIP_Surround[1] == 0){
                MKCM_WriteRegister(KCM_LISTEN_MODE, 0x10);  // 原始多声道模式
            }else if (gDIP_Surround[1] < 3){
                MKCM_WriteRegister(KCM_LISTEN_MODE, 0x20 | gDIP_Surround[1]);   // 选择多声道模式，B1:0为各种不同算法的多声道模式
            }else {
