...
CONST_CHAR TabEqSetup[4] = {                                // 多路均衡EQ音效处理设置，注意从字节1开始的
    0x03,                                                   // KCM_EQ_SETUP字节1 前置通道使用EQ
    0x00 | 5,                                               // 主声道使用5段EQ
    0xc0 | 2,                                               // 话筒声道高低音，B7=HSF B6=LSF 共2段
...
void MEQMIC_MicRestore(){									// 话筒恢复记忆
	...
	if (gDIP_MicTone[0] > 18){								// -9到+9 超出范围的处理
        gDIP_MicTone[0] = 9;                                // 设置默认值
	}
	if (gDIP_MicTone[1] > 18){								// -9到+9 超出范围的处理
    ...
    value = MKCM_ReadRegister(KCM_MIC_ECHO);	            // 话筒延迟时间及回声比例
    ...
    MKCM_WriteRegister(KCM_MIC_MIXER, (8<<4) | 8);          // 话筒声音与主声道合成比例
...
void MEQMIC_MicKcm(BYTE index, BYTE directUp){              // 话筒各种参数调节写入到KCM
    ...
    if (index < MENU_MIC_BASS){                             // 0话筒1音量 1话筒2  2回声 3重复 4延迟
        ...
        case MENU_MIC_ECHO:                                 // 话筒回声比例
            MKCM_WriteRegister(KCM_MIC_ECHO, gDIP_MicCtrl[4] << 4 | gDIP_MicCtrl[2]);   // 话筒回声及话筒多段EQ均衡音效处理选择比例
            break;
        case MENU_MIC_REPEAT:                               // 话筒重复比例
            value = 8;                                      // 直达声比例固定为8
            MKCM_WriteRegister(KCM_MIC_REPEAT, value << 4 | gDIP_MicCtrl[3]);
        break;
            default:
        break;
        }
...
void MEQMIC_MicSetTone(){                                   // 写入话筒的音调到KCM
    ...
    WORD base;
    WORD treble;
    base = makeEqValue(gDIP_MicTone[0], MIC_BASS_300HZ);
    treble = makeEqValue(gDIP_MicTone[1], MIC_TREBL_5000HZ);
    
    temp[0] = 4;                                        // 第4组为话筒的音效
    // 字节1开始每3个字节为两段12位的EQ设置值
    temp[1] = base & 0xff;                              // 第一字节为第1段低8位
    temp[2] = treble & 0xff;                            // 第二字节为第2段低8位
    temp[3] = ((base >> 8) & 0x0f) | ((treble >> 4) & 0xf0);  // 第三字节的B3:0为第1段高4位，B7:4为第2段高4位
