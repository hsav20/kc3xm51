...
InlineLexer.prototype.output = function(src) {
  ...
  // highlight
    ...
    if ( this.options.gfm && this.options.emoji) {
	    // emoji (gfm)
    ...
    if (this.options.mathjax) {
	    // mathblock
	    if (cap = this.rules.mathblock.exec(src)) {
	      src = src.substring(cap[0].length);
	      out += this.renderer.mathblock( escape(cap[1]) );
	      continue;
	    }
	    ...
	    if (cap = this.rules.mathinline.exec(src)) {
	      src = src.substring(cap[0].length);
	      out += this.renderer.mathinline( escape(cap[1]) );
	      continue;
	    }
    }
...
Renderer.prototype.image = function(href, title, text, props) {
  var out = '<img src="' + this.options.img_callback(href) + '" title="' + text + '"';
  if (title) {
    out += ' title="' + title + '"';
  }
...
Renderer.prototype.mathblock = function(text) {
 	return '<div class="mathjax">$$'
    + text
    + '$$</div>';
};

Renderer.prototype.mathinline = function(text) {
